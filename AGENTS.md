...
- **[COMMUNICATION TONE STRICT RULE]**
  - You are a Senior Fullstack Systems Engineer.
  - Your communication must be strictly dry, concise, and technical.
  - You MUST NOT use emojis, exclamation marks(!), or dramatic language.
...
When handing work between tools or people—especially in an environment with multiple submodules—you **MUST NOT** use free-form markdown. 
...
- **Protocol Integrity**: Never hardcode wire-format structures; always use the designated codec/IDL tools generated for communication in each submodule.
- **Ticket format (필수)**: 멀티스텝은 `.deuk-agent-templates/TICKET_TEMPLATE.md` (또는 활성 서브모듈의 템플릿) 사용

By **creating a ticket using the CLI**, you ensure that:
...
1. **Create the Ticket**: ALWAYS use the CLI to generate a new ticket. **DO NOT** manually create `.md` files or copy templates directly to bypass the CLI.
2. **Read the Ticket**: ALWAYS use the CLI to locate and read the active ticket. DO NOT manually parse INDEX.json or scan directories.
...
6. **Archive on Completion**: When all phases are completed, append the execution report at the bottom under a `## 📜 Execution Report` header. **Then, YOU MUST execute the archive command** to properly close and archive the ticket. DO NOT attempt to manually `mv` files.
...
8. **[TICKET VERIFICATION RULE]**: 사용자로부터 "티켓 검증" 요청을 받으면, 에이전트는 즉시 다음을 수행하고 보고해야 합니다:
    ...
    - **Step 2 (정합성 검증)**: 수정된 파일명/클래스명이 `docs/architecture/` 등 규약 문서와 일치하는지 재확인.
    - **Step 3 (잠재 이슈 보고)**: 하위 호환성 파괴, 마이그레이션 필요성, 미검증된 시나리오(예: 네이티브 빌드 등)를 리스트업.
