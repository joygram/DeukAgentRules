...
2. **Phase 1: Plan** — Run `npx deuk-agent-rule ticket create`. Search and read the project architecture rules (e.g., DOMAIN_RULES.md or *_ARCHITECTURE.md). You MUST completely fill out the **Agent Permission Contract (APC)** block in the ticket, explicitly citing the rules that govern your changes. **STOP AND WAIT for explicit user approval before writing any code.**
