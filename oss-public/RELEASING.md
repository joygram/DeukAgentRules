...
Maintainer steps for Deuk Agent Workflow npm packages:
...
6. Run `npm run publish` when ready. This publishes `deuk-agent-rule` first, then the `deuk-agent-workflow` wrapper package. Registry credentials are required.
