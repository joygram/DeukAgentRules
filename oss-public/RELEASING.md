...
3. Push `main` (and tags if you use them on this remote). **Pushing a release commit** whose message starts with `chore(release):` to **`main`** triggers [`.github/workflows/release.yml`](.github/workflows/release.yml), which runs `npm pack` and creates a **GitHub Release** with the tarball. Pushing a matching **`v*.*.*` tag** also triggers the same workflow.
