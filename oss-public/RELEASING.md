...
6. Run `npm run smoke:npm:docker` to install the packed packages in a clean Node container.
7. Run `npm run publish` when ready. This publishes `deuk-agent-flow` first, then the `deuk-agent-rule` package. Registry credentials are required.
8. If the compatibility `deuk-agent-rule` version is already published and you only need to introduce the primary package once, run `npm run publish:bootstrap:dry` and then `npm run publish:bootstrap`.
9. Optionally extend GitHub Release notes beyond the auto-generated summary.
