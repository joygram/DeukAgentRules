...
Deuk Agent Workflow npm 패키지 유지보수 절차:
...
6. 준비되면 저장소 루트에서 `npm run publish`를 실행한다. 이 명령은 `deuk-agent-rule`을 먼저 배포하고, 이어서 `deuk-agent-workflow` wrapper 패키지를 배포한다. registry credentials가 필요하다.
