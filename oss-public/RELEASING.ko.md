...
6. `npm run smoke:npm:docker`를 실행해 깨끗한 Node 컨테이너에서 packed package 설치를 검증한다.
7. 준비되면 저장소 루트에서 `npm run publish`를 실행한다. 이 명령은 `deuk-agent-flow`를 먼저 배포하고, 이어서 `deuk-agent-rule` 패키지를 배포한다. registry credentials가 필요하다.
8. 이미 현재 버전의 호환 패키지 `deuk-agent-rule`이 배포되어 있고 primary 패키지만 한 번 도입해야 한다면 `npm run publish:bootstrap:dry` 후 `npm run publish:bootstrap`을 실행한다.
9. GitHub Releases에서 자동 생성 요약 외에 설명을 추가할 수 있다.
