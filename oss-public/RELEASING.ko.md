...
3. **`main`을 푸시**(이 원격에 태그를 쓰면 태그도 함께). 커밋 메시지가 `chore(release):`로 시작하는 **릴리스 커밋**이 **`main`**에 올라오면 [`.github/workflows/release.yml`](.github/workflows/release.yml)이 돌아가 `npm pack` 후 **GitHub Release**에 tarball을 붙인다. **`v*.*.*` 태그** 푸시도 동일 워크플로를 트리거한다.
