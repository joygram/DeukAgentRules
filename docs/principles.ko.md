...
## 1. Hub-Spoke 아키텍처 (SSoT)

문서와 규칙 시스템은 단일 진실 공급원(Single Source of Truth)을 가져야 합니다.

- **Hub**: `AGENTS.md`는 모든 정본 규칙을 포함합니다.
- **Spoke**: IDE별 규칙 파일은 최소한의 포인터 역할만 수행합니다.
- **이유**: IDE마다 규칙이 파편화되면 에이전트의 행동이 일관성을 잃고 예측 불가능해집니다.
...
## 5. 서브모듈 격리 (Submodule Isolation)

에이전트의 실행은 반드시 타겟 서브모듈 범위 내로 제한되어야 합니다.
