...
- **메커니즘**: Phase 1에서 티켓을 발행하고 APC(Agent Permission Contract)와 planLink를 채워 스코프와 판단 근거를 검색 가능한 형태로 남깁니다. 사용자가 실행을 명확히 요청했고 Phase 1 기록이 완성되어 있으면 Phase 2로 이어갈 수 있습니다.
...
- **메커니즘**: 코드를 작성하기 전 티켓의 APC 블록(`[BOUNDARY]`, `[CONTRACT]`, `[PATCH PLAN]`)과 planLink를 채웁니다. 기존 계획이 있다면 `--from-plan`으로 티켓화할 수 있습니다.
