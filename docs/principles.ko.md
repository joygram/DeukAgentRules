...
- **메커니즘**: Phase 1에서 티켓을 발행하거나 기존 티켓을 선택하고 APC(Agent Permission Contract)와 planLink를 채웁니다. 진행 가능한 티켓이 없을 때는 후속 티켓을 만들기 전에 최근 git history를 분석합니다. 티켓은 스코프, 계약, 진행 체크를 맡고, planLink는 에이전트의 서술형 문제 분석, 원인 가설, 결정 근거, 실행 전략, 검증 설계를 맡습니다. planLink에는 진행 체크박스를 두지 않습니다.
