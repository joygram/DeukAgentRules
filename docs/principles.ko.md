...
- **메커니즘**: Phase 1에서 티켓을 발행하고 APC(Agent Permission Contract)와 planLink를 채웁니다. 티켓은 스코프와 계약을, planLink는 근거와 실행/검증 기록을 맡으며 서로 같은 내용을 반복하지 않습니다. 사용자가 실행을 명확히 요청했고 Phase 1 기록이 완성되어 있으면 Phase 2로 이어갈 수 있습니다.
...
- **메커니즘**: 코드를 작성하기 전 티켓의 APC 블록(`[BOUNDARY]`, `[CONTRACT]`, `[PATCH PLAN]`)과 planLink를 채웁니다. APC는 경계와 계약만 담고, 세부 실행 근거와 검증은 planLink에만 둡니다.
