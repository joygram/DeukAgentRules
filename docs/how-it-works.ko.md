...
2. **APC 및 planLink 기록**: 에이전트는 코드를 수정하기 전, 티켓에 명시된 APC(Agent Permission Contract)의 `[BOUNDARY]`, `[CONTRACT]`, `[PATCH PLAN]`과 planLink를 채웁니다. 티켓은 스코프/계약을, planLink는 문제 분석/원인 가설/결정 근거/실행 전략/검증 설계를 맡으며 같은 내용을 반복하지 않습니다.
