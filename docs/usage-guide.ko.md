...
이 가이드는 DeukAgentFlow를 실제 프로젝트에 배포하고 에이전트와 함께 효율적으로 사용하는 방법을 단계별로 설명합니다. 규칙의 최종 권한은 `AGENTS.md`와 각 프로젝트의 `PROJECT_RULE.md`에 있고, 이 문서는 그 동작을 설명하는 안내서입니다.
...
즉, 이 문서의 설명은 프로젝트 루트가 가진 실제 규칙을 돕기 위한 것이고, 충돌이 나면 항상 프로젝트 규칙과 코어 룰이 먼저입니다.
